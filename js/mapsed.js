...
(function () {
  ...
  $.fn.mapsed = function (options) {
    ...
    function hideEmpty(model, $vw) {
      ...
      $vw
        .find(".mapsed-street")
        .toggle(model.street && model.street.length > 0);
      $vw.find(".mapsed-town").toggle(model.town && model.town.length > 0);
      $vw.find(".mapsed-area").toggle(model.area && model.area.length > 0);
      $vw
        .find(".mapsed-postCode")
        .toggle(model.postCode && model.postCode.length > 0);
      $vw
        .find(".mapsed-country")
        .toggle(model.country && model.country.length > 0);
    ...
    var ctor = function () {
      ...
      if (settings.onSelect) {
        _mapContainer.on("click", "button.mapsed-select-button", function () {
          var element = $(this);
          onPlaceSelect(element);
        });
      }
      if (settings.onSave) {
        _mapContainer.on("click", "button.mapsed-save-button", function () {
          var element = $(this);
          onPlaceSave(element);
        });
        // only allow edit if the user can actually save the result!
        _mapContainer.on("click", "button.mapsed-edit-button", function () {
          var element = $(this);
          onPlaceEdit(element);
        });
      }
      if (settings.onDelete) {
        _mapContainer.on("click", "button.mapsed-delete-button", function () {
          var element = $(this);
